...
var ministatementService = require('./services/ministatementService');
...
//var fs = require('fs');
//console.log('our exhange:' + global.gConfig.amqp_exchange );
...
amqp.connect(global.gConfig.amqp_url).then(function(conn) {
  ...
  return conn.createChannel().then(function(ch) {
    ...
    ok = ok.then(function() {
      return ch.assertQueue(global.gConfig.ministatementQue, {exclusive: false});
    });
    ok = ok.then(function(qok) {
      return ch.bindQueue(qok.queue, global.gConfig.amqp_exchange, 'ministatement').then(function() {
        return qok.queue;
      });
    });
    ...
    return ok.then(function() {
      console.log(' [*] Waiting for ministatement messages. To exit press CTRL+C');
    });
    ...
    function getRequestParams(msg) {
        ...
        try {
          ...
          var status = ministatementService.callInitiateMiniStatement(jsonObj.transactionId, jsonObj.transactionDate, jsonObj.serviceId, jsonObj.msisdn,
