...
var topUpService = require('./services/airtimetopupService');
...
amqp.connect(global.gConfig.amqp_url).then(function(conn) {
  ...
  return conn.createChannel().then(function(ch) {
    ...
    ok = ok.then(function() {
      return ch.assertQueue(global.gConfig.airtimeTopUpQue, {exclusive: false});
    });
    ok = ok.then(function(qok) {
      return ch.bindQueue(qok.queue, global.gConfig.amqp_exchange, 'payment').then(function() {
        return qok.queue;
      });
    });
    ...
    return ok.then(function() {
      console.log(' [*] Waiting for airtimetopup messages. To exit press CTRL+C');
    });
    ...
    function getRequestParams(msg) {
        ...
        try {
          ...
          var status = topUpService.callInitiatePayment(jsonObj.transactionId, jsonObj.transactionDate, jsonObj.serviceId,
             jsonObj.msisdn, jsonObj.receiverMsisdn, jsonObj.amount, jsonObj.messageType, jsonObj.conversationID, 
