...
amqp.connect(global.gConfig.amqp_url).then(function(conn) {
  ...
  return conn.createChannel().then(function(ch) {
    ...
    ok = ok.then(function() {
      return ch.assertQueue(global.gConfig.balanceEnquiryQue, {exclusive: false});
    });
    ok = ok.then(function(qok) {
      return ch.bindQueue(qok.queue, global.gConfig.amqp_exchange, 'balanceenq').then(function() {
        return qok.queue;
      });
    });
    ...
    return ok.then(function() {
      console.log(' [*] Waiting for balanceEnquiry messages. To exit press CTRL+C');
    });
